#!/bin/bash
# Retrex Masternode Setup Script V1.4 for Ubuntu 16.04 LTS
#
...
function stop_daemon {
    if pgrep -x 'retrexd' > /dev/null; then
        echo -e "${YELLOW}Attempting to stop Retrexd${NC}"
        retrex-cli stop
        sleep 30
        if pgrep -x 'retrex' > /dev/null; then
            echo -e "${RED}Retrexd daemon is still running!${NC} \a"
            echo -e "${RED}Attempting to kill...${NC}"
            pkill retrexd
            sleep 30
            if pgrep -x 'retrexd' > /dev/null; then
                echo -e "${RED}Can't stop retrexd! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
echo -e "${YELLOW}Retrex Masternode Setup Script V1.5 for Ubuntu 16.04 LTS${NC}"
...
wget https://github.com/KieranBry/Retrex/releases/download/v1.0.1.0/retrex-1.0.1-ubuntu-compiled-16.04.tar.gz
...
rm -rf retrex-1.0.1-ubuntu-compiled-16.04.tar.gz
 ...
 sudo cp ~/RetrexMasternodeSetup/retrex/retrex* /usr/bin/
 sudo chmod 755 -R ~/RetrexMasternodeSetup
 sudo chmod 755 /usr/bin/retrex*

 # Deploy masternode monitoring script
 cp ~/RetrexMasternodeSetup/retrexmon.sh /usr/local/bin
 sudo chmod 711 /usr/local/bin/retrexmon.sh

 #Create datadir
 if [ ! -f ~/.retrex/retrex.conf ]; then
 	sudo mkdir ~/.retrex
 fi
clear
echo -e "${YELLOW}Creating retrex.conf...${NC}"
...
EOF
    ...
    sudo chmod 755 -R ~/.retrex/retrex.conf
...
echo -ne '\n'
    ...
    genkey=$(retrex-cli masternode genkey)
    if [ -z "$genkey" ]; then
    ...
    #Stopping daemon to create retrex.conf
    stop_daemon
    sleep .5
fi
# Create retrex.conf
...
#Finally, starting daemon with new retrex.conf
retrexd --daemon
sleep 5

#Setting auto start cron job for retrexd
cronjob="@reboot sleep 30 && retrexd"
crontab -l > tempcron
...
To introduce your new masternode to the Retrex network, you need to
...
Currently your masternode is syncing with the Retrex network...
...
========================================================================
To view masternode configuration produced by this script in retrex.conf:
${YELLOW}cat ~/.retrex/retrex.conf${NC}
Here is your retrex.conf generated by this script:
-------------------------------------------------${YELLOW}"
cat ~/.retrex/retrex.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit retrex.conf, first stop the retrexd daemon,
then edit the retrex.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the retrexd daemon back up:
             to stop:   ${YELLOW}retrex-cli stop${NC}
             to edit:   ${YELLOW}nano ~/.retrex/retrex.conf${NC}
             to start:  ${YELLOW}retrexd${NC}
========================================================================
To view retrex debug log showing all MN network activity in realtime:
             ${YELLOW}tail -f ~/.retrex/debug.log${NC}
========================================================================
...
sync status etc. in real-time, run the nodemon.sh script:
                 ${YELLOW}retrexmon.sh${NC}
or just type 'node' and hit <TAB> to auto-complete script name.
========================================================================
Enjoy your Retrex Masternode and thanks for using this setup script!
"
sleep 30
# Run retrexmon.sh
retrexmon.sh
